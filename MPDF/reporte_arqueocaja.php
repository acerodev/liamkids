...
$query = "SELECT caja.caja_id, 
				...
				empresa.email,
				empresa.moneda,
				(SELECT  IFNULL(SUM(monto_efectivo),0) from venta_cabecera where  f_pago = 'Efectivo' and venta_estado = 'REGISTRADA' and caja_id =  '".$_GET['codigo']."' ) as efectivo,
				(SELECT  IFNULL(SUM(monto_transfe),0) from venta_cabecera where  f_pago = 'Yape' and venta_estado = 'REGISTRADA' and caja_id =  '".$_GET['codigo']."') as yape,
				(SELECT  IFNULL(SUM(monto_transfe),0) from venta_cabecera where  f_pago = 'Plin' and venta_estado = 'REGISTRADA' and caja_id =  '".$_GET['codigo']."') as plin,
				(SELECT  IFNULL(SUM(monto_transfe),0) from venta_cabecera where  f_pago = 'Transferencia' and venta_estado = 'REGISTRADA' and caja_id =  '".$_GET['codigo']."') as transfe,
